...
def coerce_numerics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Handles South African decimal format (comma as decimal separator, e.g. 0,26).
    """
    for col in df.columns:
        s = df[col]
        # Replace SA comma-decimals with dots before converting (digit,digit pattern only)
        if hasattr(s, "str"):
...
def cross_impute(freq: pd.DataFrame, sev: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for col in SHARED_COLS:
        ...
        if freq_col in sev_merged.columns:
            ...
            if fill_mask.any():
                fill_vals = sev_merged.loc[fill_mask, freq_col]
                # cast to target column dtype to satisfy pandas 3.x strict typing
                try:
                    fill_vals = fill_vals.astype(sev_merged[col].dtype)
                except (ValueError, TypeError):
                    fill_vals = fill_vals.astype(object)
                sev_merged.loc[fill_mask, col] = fill_vals
    ...
    for col in SHARED_COLS:
        ...
        if sev_col in freq_merged.columns:
            ...
            if fill_mask.any():
                fill_vals = freq_merged.loc[fill_mask, sev_col]
                try:
                    fill_vals = fill_vals.astype(freq_merged[col].dtype)
                except (ValueError, TypeError):
                    fill_vals = fill_vals.astype(object)
                freq_merged.loc[fill_mask, col] = fill_vals
